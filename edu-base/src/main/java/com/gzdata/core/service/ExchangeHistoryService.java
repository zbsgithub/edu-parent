                                                                        
package com.gzdata.core.service;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import com.gzdata.common.db.mybatis.dao.BaseDAOInterface;
import com.gzdata.common.db.mybatis.service.AbstractBaseService;
import com.gzdata.core.dao.ExchangeHistoryDao;
import com.gzdata.core.model.ExchangeHistory;

/**
 * 
 * 说明：处理对兑换历史的业务操作
 * 
 * @author 张兵帅
 * 
 * @version 1.0
 * 
 * @since 2021年01月15日
 */
@Service
public class ExchangeHistoryService extends AbstractBaseService<ExchangeHistory> {

	@Autowired
	private ExchangeHistoryDao exchangeHistoryDao;
	
	 
	@Override
	protected BaseDAOInterface<ExchangeHistory> getDAO() {
		return exchangeHistoryDao;
	}

}